...
const bot = client
    ...
    message.channel.send(new Discord.RichEmbed()
        ...
        Você tem ${gameconfig.tries} chances de acertá-lo.
    ...
    game_collector.on('collect', u_msg => {
        ...
        if (parseInt(u_msg.content) < random_number) {
            ...
            gameconfig.tries--;
            return message.channel.send(new Discord.RichEmbed()
                .setTitle(`**${u_msg.content}** é **menor** que o número secreto, tente novamente.`)
        ...
        if (parseInt(u_msg.content) > random_number) {
            ...
            gameconfig.tries--;
            return message.channel.send(new Discord.RichEmbed()
                .setTitle(`**${u_msg.content}** é **maior** que o número secreto, tente novamente.`)
                .setDescription(`${gameconfig.tries} ${tryplural}.`)
                .setColor(color))
        }
    })
    ...
    game_collector.on('end', (msg, reason) => {
        ...
        switch (reason) {
            ...
            case 'forced':
                return message.channel.send(new Discord.RichEmbed()
                    .setTitle('Game Over!')
                    .setDescription(`Jogo finalizado
                    O número secreto era **${random_number}**.`)
                    .setColor('#FF0000'));
            case 'win':
                return message.channel.send(new Discord.RichEmbed()
